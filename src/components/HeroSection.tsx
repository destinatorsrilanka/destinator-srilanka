...
export default function HeroSection() {
  ...
  return (
    ...
    <section className="relative min-h-screen w-full bg-black font-montserrat flex flex-col overflow-x-hidden">
      ...
      {/* --- Main UI Layer --- */}
      <div className="relative z-30 flex flex-col flex-1 px-6 md:px-12 lg:px-24 overflow-x-hidden overflow-y-auto scrollbar-hide">
        {/* Navbar Safe Zone */}
        <div className="h-[100px] md:h-[130px] shrink-0" />

        {/* Content Group */}
        <div className="flex-1 flex flex-col justify-end min-h-[450px]">
          <div className="max-w-full md:max-w-5xl animate-slide-up pb-6 md:pb-10">
            <div className="flex flex-col md:flex-row md:items-end gap-4 md:gap-8 mb-6">
              ...
              <div className="relative w-fit shrink-0">
                <div className="relative px-5 py-3 md:px-8 md:py-4 bg-white/5 backdrop-blur-md border border-white/10 overflow-hidden rounded-2xl flex flex-col items-center">
                  ...
                  <h3 className="text-destinator-orange text-[2.5vh] md:text-3xl lg:text-4xl font-black tracking-[0.15em] md:tracking-[0.2em] uppercase italic flex items-center gap-0 leading-none mb-2">
                    DESTINAT
            ...
            {/* Title & Description */}
            <div className="mb-4">
              <h2 className="text-destinator-orange text-[1.4vh] md:text-lg lg:text-xl font-bold italic mb-1">
                {carouselImages[currentIndex]?.frTitle}
              </h2>
              <h1 className="text-white text-[4vh] md:text-5xl lg:text-6xl xl:text-7xl font-black uppercase whitespace-normal md:whitespace-nowrap leading-[1.1]">
              ...
              <div className="flex gap-3 p-2 bg-black/40 backdrop-blur-xl rounded-[2.5rem] border border-white/20">
                {carouselImages.map((item, index) => (
                  <button
                    ...
                    className={`relative w-14 h-9 md:w-24 md:h-14 lg:w-28 lg:h-16 rounded-xl overflow-hidden border-2 transition-all ${index === currentIndex ? "border-destinator-orange scale-105" : "border-transparent opacity-40"}`}
                  >
                    <Image
                      src={item.src}
                      alt="nav"
